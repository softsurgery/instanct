...
currentUserId?: string; // <- passer l'utilisateur courant
}

export const BaseProfile = ({ className, isFetchUserPending, currentUserId }: BaseProfileProps) => {
...
const { followers, followings, isFollowersPending, isFollowingPending } = useFollowSystem({
id: user?.id || "",
...
{ value: "conversations", 
  ...
  icon: MessageSquare, 
  content: <div>Conversations Content
...
const filteredTabs = tabs.filter(tab => tab.value !== "notifications" || user?.id !== currentUserId);
...
<a href={`mailto:${user?.email}`}>{user?.email || "No email"}
  ...
  {/* Tabs */}
  <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col flex-1 overflow-auto">
    <TabsList
  className={`grid mb-4`}
